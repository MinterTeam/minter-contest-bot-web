    ...
    export default {
        ...
        computed: {
            ...
            message() {
                ...
                let errorText = this.error.description;
                if (!errorText) {
                    errorText = getErrorText(this.error, '');
                }
    ...
    <div class="main-content--error">
        <div class="error__container u-container u-container--small u-section">
            <div class="error__inner">
                <h1 class="error__title u-h1">
                    <template v-if="error.title">{{ error.title }}</template>
                    <template v-else>Error {{ statusCode }}</template>
                </h1>
                <p class="error__description" v-if="statusCode === 404">We couldn't find the page you're looking for.</p>
                <p class="error__description" v-else-if="statusCode === 502">Service is unavailable: {{ error.message }}</p>
